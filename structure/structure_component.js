...
exports.newPrint = function (attr, callback) {
    ...
    translateHelper.writeLocales(idApp, "component", componentName, showComponentName, attr.googleTranslate, function () {
        ...
        domHelper.read(showFieldsPath).then(function ($) {
            ...
            componentContent += "<div id='" + componentNameLower + "' class='tab-pane fade'>\n";
            ...
            componentContent += "		}";
            componentContent += "		#" + componentNameLower + "-content,";
            componentContent += "		#" + componentNameLower + "-content * {";
            ...
            componentContent += "			overflow: visible;";
            componentContent += "		}";
            componentContent += "		#" + componentNameLower + "{";
            componentContent += "			height: 100%;";
            componentContent += "			overflow: visible;";
            componentContent += "		}";
            ...
            componentContent += "	}";
            componentContent += "</style>";
            componentContent += "	<button data-component='" + componentNameLower + "' class='component-print-button'><i class='fa fa-print' aria-hidden='true' style='margin-right:5px;'></i><!--{@__ key=\"global_component.print.action\"/}--></button>\n";
            componentContent += "	<div id='" + componentName + "-content' class='print-tab'>\n";
...
exports.addNewComponentAdress = function (attr, callback) {
    try {
        ...
        routeContent = routeContent.replace(/MODEL_NAME/g, 'C_adress_' + attr.id_data_entity);
        fs.writeFileSync(application_path + 'routes/' + componentUrl + '.js', routeContent, 'utf8');
        //generate views data
        var fields = adress_utils.generateFields();
        var createFile = fs.readFileSync(c_adress_views_path + 'create.dust', 'utf8');
        var updateFile = fs.readFileSync(c_adress_views_path + 'update.dust', 'utf8');
        ...
        //copy public js adress component file
        fs.copySync(c_adress_views_path + '../public/c_adress.js', application_path + 'public/js/c_adress.js');
        //create adress views files
        fs.copySync(c_adress_views_path, application_path + 'views/' + componentName + '/');
        ...
        langFR.component['c_adress'] = fields.locales.fr;
        langEN.component['c_adress'] = fields.locales.en;
        ...
        setupComponentModel(attr.id_application, 'adress', componentName, 'adress', function () {
            //add new js inclusion in main_layout
            domHelper.read(application_path + 'views/main_layout.dust').then(function ($layout) {
                var all_include = $layout('#include_js');
                //if don't exist
                if (all_include.html().indexOf('/js/c_adress.js') < 0)
                    $layout('#include_js').append('<script src="/js/c_adress.js" type="text/javascript"></script>');
                domHelper.writeMainLayout(application_path + 'views/main_layout.dust', $layout).then(function () {
                    callback(null);
                }).catch(function (e) {
                    return callback(e);
                });
