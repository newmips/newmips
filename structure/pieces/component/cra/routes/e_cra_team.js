    ...
    filterDataTable("E_cra_team", req.body, include).then(function (data) {
        ...
        if (entity_helper.status.statusFieldList(attributes).length > 0)
            for (var i = 0; i < data.data.length; i++)
                statusPromises.push(entity_helper.status.currentStatus(models, "e_cra_team", data.data[i], attributes, req.session.lang_user));
                ...
                for (var field in data.data[i].dataValues) {
                    ...
                    if (typeof attributes[field] != 'undefined' && attributes[field].newmipsType == 'picture' && value != null) {
                        ...
                        if (partOfFile.length > 1) {
                            ...
                            var filePath = thumbnailFolder + 'e_cra_team/' + partOfFile[0] + '/' + value;
                            todo.push({
                                value: value,
                                file: filePath,
                                field: field,
...
router.post('/search', block_access.actionAccessMiddleware('cra_team','read'), function(req, res) {
    ...
    models.E_cra_team.findAll(where).then(function(results) {
        res.json(results);
    }).catch(function(e) {
        console.error(e);
