    ...
    filterDataTable("E_translation", req.body, include).then(function (data) {
        ...
        if (entity_helper.status.statusFieldList(attributes).length > 0)
            for (var i = 0; i < data.data.length; i++)
                statusPromises.push(entity_helper.status.currentStatus(models, "e_translation", data.data[i], attributes, req.session.lang_user));
...
router.post('/search', block_access.actionAccessMiddleware('translation','read'), function(req, res) {
    ...
    models.E_translation.findAll(where).then(function(results) {
        res.json(results);
    }).catch(function(e) {
        console.error(e);
